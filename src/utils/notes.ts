...
export async function getBookmarks(): Promise<Bookmark[]> {
  ...
  return notes
    .filter(note => 
      note.data.url && 
      note.data.tags?.some(tag => tag === 'type/resource')
    )
    .map(note => ({
...
export async function getRecentNotes(limit: number = 5): Promise<NoteCollectionEntry[]> {
  ...
  const notesWithModified = notes.map(note => ({
    note,
    lastModified: getGitLastModified(`src/content/wiki/${note.slug}.md`)
  }));
...
export function resolveWikiLink(target: string, allNotes: NoteCollectionEntry[]): { exists: boolean; slug?: string } {
  ...
  const match = allNotes.find(note => {
    ...
    return title === normalizedTarget ||
           ...
           slug.endsWith('/' + normalizedTarget) ||
